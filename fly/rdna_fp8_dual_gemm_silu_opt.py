"""Optimised fused dual FP8 GEMM + silu_mul for RDNA4 (gfx120x, wave32).

Same interface as rdna_fp8_dual_gemm_silu, two changes for compute-bound shapes:
...
2. k_unroll=2 for all M — the original uses k_unroll=1 for M>=256 (enough
   tile-level parallelism), but the dual-GEMM loop body is 2x bigger so the
   extra unrolling still pays off for pipeline fill.
"""
...
# =============================================================================
# Host-side preshuffle helpers  (identical to rdna_fp8_dual_gemm_silu)
# =============================================================================
...
def compile_fp8_dual_gemm_silu_opt(
    *,
    M: int,
    N: int,
    K: int,
    ...
    def launch_dual_gemm_silu_opt(
        ...
        launcher = kernel_dual_gemm_silu_opt(
    ...
    return launch_dual_gemm_silu_opt


__all__ = [
    "compile_fp8_dual_gemm_silu_opt",
    "preshuffle_b_fp8",
