"""Export rdna_fp8_dual_gemm_silu_opt as HSACO + C header.

Optimised dual fp8 GEMM + silu_mul for RDNA4 (tile_n=128, interleaved WMMA).

Run with:
    python fly/export_fp8_dual_gemm_silu_opt.py                       # defaults
    python fly/export_fp8_dual_gemm_silu_opt.py --N 2048 --K 4096    # single config
    python fly/export_fp8_dual_gemm_silu_opt.py --config fly/export_configs/rdna_fp8_dual_gemm_silu.toml

Outputs per config (in fly/artifacts/):
    rdna_fp8_dual_gemm_silu_opt_N{N}_K{K}_TM{tile_m}_TN{tile_n}.hsaco + .h
"""
import argparse
import os
import re
...
from rdna_fp8_dual_gemm_silu_opt import compile_fp8_dual_gemm_silu_opt


# ── Tile / wave config (mirrors compile_fp8_dual_gemm_silu_opt) ───────────────

def _compute_config(M: int, N: int, K: int, tile_m: int = 32) -> dict:
    tile_n = 128  # fixed: tile_n=256 overflows 256-VGPR limit with dual accumulators
...
def _compile_and_get_ir(M: int, N: int, K: int) -> str:
    launcher = compile_fp8_dual_gemm_silu_opt(M=M, N=N, K=K)
...
_HEADER = """\
/* Auto-generated by export_fp8_dual_gemm_silu_opt.py — do not edit.
 *
 * HSACO ELF for {kernel_name} (gfx1201, wave32/RDNA4).
 *
 * Optimised dual fp8 GEMM + silu_mul: interleaved WMMA, tile_n=128.
 * Computes: C[M,N] = silu(A @ B_gate) * (A @ B_up)
 * where A, B_gate, B_up are fp8_e4m3fn; output C is fp16.
 *
 * Fixed dimensions: N={N}, K={K}
 *   A:           fp8_e4m3fn  [M, {K}]
 ...
 *
 * Grid:  (M / FP8_DUAL_SILU_OPT_TILE_M) * ({N} / {tile_n}) x 1 x 1
...
#define FP8_DUAL_SILU_OPT_N                  {N}
#define FP8_DUAL_SILU_OPT_K                  {K}
#define FP8_DUAL_SILU_OPT_TILE_M             {tile_m}
#define FP8_DUAL_SILU_OPT_TILE_N             {tile_n}
#define FP8_DUAL_SILU_OPT_THREADS_PER_BLOCK  {threads_per_block}

typedef struct {{
    hipModule_t   module;
    hipFunction_t func;
}} fp8_dual_silu_opt_Module_t;

static inline int fp8_dual_silu_opt_Module_Load(fp8_dual_silu_opt_Module_t *m, const char *hsaco_path) {{
    hipError_t err = hipModuleLoad(&m->module, hsaco_path);
    if (err != hipSuccess) {{
        fprintf(stderr, "fp8_dual_silu_opt: hipModuleLoad(%s): %s\\n", hsaco_path, hipGetErrorString(err));
        return (int)err;
    }}
    err = hipModuleGetFunction(&m->func, m->module, "{kernel_name}");
    if (err != hipSuccess) {{
        fprintf(stderr, "fp8_dual_silu_opt: hipModuleGetFunction: %s\\n", hipGetErrorString(err));
...
static inline void fp8_dual_silu_opt_Module_Unload(fp8_dual_silu_opt_Module_t *m) {{
    if (m->module) (void)hipModuleUnload(m->module);
    m->module = NULL;
    m->func   = NULL;
}}

/* Launch the optimised dual-GEMM+silu_mul kernel.
 * M must be a runtime multiple of FP8_DUAL_SILU_OPT_TILE_M; N and K are fixed at export time.
 * d_B_gate_shuf and d_B_up_shuf must be in preshuffled layout [{N_div16},{K_div16},2,16,8].
 * Output d_C is fp16 [M, {N}]. */
static inline int fp8_dual_silu_opt_wrapper(
        const fp8_dual_silu_opt_Module_t *m,
        ...
        int32_t M,
        hipStream_t stream) {{
    if (M % FP8_DUAL_SILU_OPT_TILE_M != 0) {{
        fprintf(stderr, "fp8_dual_silu_opt: M=%d not divisible by tile_m=%d\\n", M, FP8_DUAL_SILU_OPT_TILE_M);
        return -1;
    }}
    int32_t grid_m = M / FP8_DUAL_SILU_OPT_TILE_M;
    unsigned int total_blocks = (unsigned int)grid_m * ({N} / FP8_DUAL_SILU_OPT_TILE_N);
    void *args[] = {{ &d_C, &d_A, &d_B_gate_shuf, &d_B_up_shuf,
                     &d_scale_a, &d_scale_b_gate, &d_scale_b_up, &grid_m }};
    hipError_t err = hipModuleLaunchKernel(
            m->func,
            total_blocks, 1, 1,
            FP8_DUAL_SILU_OPT_THREADS_PER_BLOCK, 1, 1,
            0, stream, args, NULL);
    if (err != hipSuccess) {{
        fprintf(stderr, "fp8_dual_silu_opt: hipModuleLaunchKernel: %s\\n", hipGetErrorString(err));
        return (int)err;
    }}
    return 0;
}}
...
def _export_one(M: int, N: int, K: int, artifacts_dir: str) -> None:
    ...
    name = f"rdna_fp8_dual_gemm_silu_opt_N{N}_K{K}_TM{cfg['tile_m']}_TN{cfg['tile_n']}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Export optimised fp8 dual-GEMM+silu_mul kernel as HSACO + C header."
    )
    parser.add_argument("--config", metavar="FILE",
                        help="TOML config file with a list of M,N,K configs to batch-export")
    parser.add_argument("--M", type=int, default=131072, help="Batch dimension (default: 131072)")
    parser.add_argument("--N", type=int, default=2048,   help="Output (inter_dim) dimension (default: 2048)")
    parser.add_argument("--K", type=int, default=4096,   help="Input (model_dim) dimension (default: 4096)")
